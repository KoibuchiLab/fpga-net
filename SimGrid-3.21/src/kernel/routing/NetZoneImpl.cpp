...
{
	this->get_bypass_route(src,dst,-1,links,latency);
}
bool NetZoneImpl::get_bypass_route(routing::NetPoint* src, routing::NetPoint* dst,double size,
                                   /* OUT */ std::vector<resource::LinkImpl*>& links, double* latency)
{
  ...
  if (bypassedRoute) {
    ...
    if (src != key.first)
      get_global_route(src, bypassedRoute->gw_src,size, links, latency);
    for (resource::LinkImpl* const& link : bypassedRoute->links) {
      links.push_back(link);
      if (latency)
        *latency += link->get_latency();
    ...
    if (dst != key.second)
      get_global_route(bypassedRoute->gw_dst, dst,size, links, latency);
    return true;
  }
...
void NetZoneImpl::get_global_route(NetPoint* src, NetPoint* dst,
                                   /* OUT */ std::vector<resource::LinkImpl*>& links, double* latency)
{
	NetZoneImpl::get_global_route(src,dst,-1,links,latency);
}
void NetZoneImpl::get_global_route(NetPoint* src, NetPoint* dst,double size,
                                   /* OUT */ std::vector<resource::LinkImpl*>& links, double* latency)
{
  ...
  if (common_ancestor->get_bypass_route(src, dst,size, links, latency))
    return;

  /* If src and dst are in the same netzone, life is good */
  if (src_ancestor == dst_ancestor) { /* SURF_ROUTING_BASE */
    route.link_list = std::move(links);
    common_ancestor->get_local_route(src, dst,size, &route, latency);
  ...
  common_ancestor->get_local_route(src_ancestor->netpoint_, dst_ancestor->netpoint_,size, &route, latency);
  xbt_assert((route.gw_src != nullptr) && (route.gw_dst != nullptr), "bad gateways for route from \"%s\" to \"%s\"",
             src->get_cname(), dst->get_cname());

  /* If source gateway is not our source, we have to recursively find our way up to this point */
  if (src != route.gw_src)
    get_global_route(src, route.gw_src,size, links, latency);
  for (auto const& link : route.link_list)
    links.push_back(link);

  /* If dest gateway is not our destination, we have to recursively find our way from this point */
  if (route.gw_dst != dst)
    get_global_route(route.gw_dst, dst, size, links, latency);
}
}
}
} // namespace
