...
{
	 this->route_to(dest,-1, links, latency);
}
void Host::route_to(Host* dest,double size,std::vector<Link*>& links, double* latency)
{
  std::vector<kernel::resource::LinkImpl*> linkImpls;
  this->route_to(dest,size, linkImpls, latency);
...
void Host::route_to(Host* dest,double size, std::vector<kernel::resource::LinkImpl*>& links, double* latency)
{
  simgrid::kernel::routing::NetZoneImpl::get_global_route(pimpl_netpoint, dest->pimpl_netpoint,size, links, latency);
