...
void sg_config_init(int *argc, char **argv)
{
  ...
  simgrid::config::declare_flag<double>(
      "network/optelec_routing_threshold",
      "Maximal size of messages (in byte) that are to be sent in electrical paths. Higher than this message will be use optical path", 
	  8192);
...
/*NguyenTT*/
  simgrid::config::declare_flag<int>("smpi/process_of_node", "NTT-Allreduce: Number of processes per a computing node", 1);
  simgrid::config::declare_flag<int>("smpi/process_of_switch", "NTT-Allreduce: Number of processes per a switch", 1);
/*END NguyenTT*/
#endif // HAVE_SMPI
