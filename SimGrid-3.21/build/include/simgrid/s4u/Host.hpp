...
public:
  ...
  void route_to(Host* dest,double size, std::vector<Link*>& links, double* latency);
  void route_to(Host* dest,double size, std::vector<kernel::resource::LinkImpl*>& links, double* latency);
  ...
  XBT_ATTRIB_DEPRECATED_v323("Please use Host::route_to()") void routeTo(Host* dest,double size, std::vector<Link*>& links,
                                                                         double* latency)
  {
	if (size <=0){
		route_to(dest, links, latency);
	} else {
		route_to(dest, size, links, latency);
	}
  }
  /** @deprecated See Host::route_to() */
  XBT_ATTRIB_DEPRECATED_v323("Please use Host::route_to()") void routeTo(
      Host* dest,double size, std::vector<kernel::resource::LinkImpl*>& links, double* latency)
  {
	if (size <=0){
		route_to(dest, links, latency);
	} else {
