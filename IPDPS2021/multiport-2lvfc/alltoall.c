...
//	if (rank == 0) printf("Chunksize: %d\n", chunksize);
	...
	for (int i = 0; i < numofnodesingroup; i++){
		if (i != nodenumber){
			...
			MPI_Irecv(recvbufintra[i], numofgroups*NUM_ITEMS, KIM_DATA, source, source, MPI_COMM_WORLD, &(reqrecvs[i]));
		}
	}
	...
	for (int i = 0; i < numofnodesingroup; i++){
		if (i != nodenumber){
			...
			for (int j = 0; j < numofgroups; j++){
				...
				// }
				memcpy(&sendbufintra[i][j * NUM_ITEMS], &data[index * NUM_ITEMS], NUM_ITEMS*sizeof(KIM_data));
			}
		...
		if (i != nodenumber){
			for(int j = 0; j < numofgroups; j++){
				if (j == groupnumber){
					...
					// }
					memcpy(&alltoallresult[dataindex * NUM_ITEMS], &recvbufintra[i][j * NUM_ITEMS], NUM_ITEMS*sizeof(KIM_data));
				} else {
