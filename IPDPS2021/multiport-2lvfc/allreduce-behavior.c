...
int main(int argc, char* argv[])
{
	...
	float* data = SMPI_SHARED_MALLOC(sizeof(float) * NUM_ITEMS_ROUND);
	...
	float* intergroupreductionresult = (float*)SMPI_SHARED_MALLOC(sizeof(float) * numofitemsinsecondreduction);
	...
	float* intergroupbuffer_ = (float*)SMPI_SHARED_MALLOC(sizeof(float) * numofgroups * numofitemsinsecondreduction);
#if defined(TIME_FOR_EACH_STEP)
	MPI_Barrier(MPI_COMM_WORLD);
	dblasttimer = MPI_Wtime();
#endif
	...
	//Final result
	float* allreduceresult = (float*)SMPI_SHARED_MALLOC(sizeof(float) * NUM_ITEMS_ROUND);
