...
int main(int argc, char * argv[]) {
	...
	MPI_Init( & argc, & argv);
	//printf("This is Kim from earth: %d", 17/4);
	...
	if (strcmp(bcast_implementation_name, "naive_bcast") &&
		...
		strcmp(bcast_implementation_name, "multiport2lvfc") &&
	...
	if (strcmp(bcast_implementation_name, "naive_bcast") == 0) {
		if (rank == 0) {
			for (int i = 1; i < num_procs; i++) {
				MPI_Send(buffer, NUM_BYTES, MPI_CHAR, i, 1, MPI_COMM_WORLD);
			}
		} else {
			MPI_Recv(buffer, NUM_BYTES, MPI_CHAR, 0, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}
	} else if (strcmp(bcast_implementation_name, "ring_bcast") == 0) {
		if (rank == 0) {
			MPI_Send(buffer, NUM_BYTES, MPI_CHAR, 1, 1, MPI_COMM_WORLD);
			MPI_Recv(buffer, NUM_BYTES, MPI_CHAR, num_procs - 1, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

		} else {
			MPI_Recv(buffer, NUM_BYTES, MPI_CHAR, rank - 1, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			MPI_Send(buffer, NUM_BYTES, MPI_CHAR, (rank + 1) % num_procs, 1, MPI_COMM_WORLD);
		}
	} else if (strcmp(bcast_implementation_name, "default_bcast") == 0) {
		MPI_Bcast(buffer, NUM_BYTES, MPI_CHAR, 0, MPI_COMM_WORLD);
	} else if (strcmp(bcast_implementation_name, "pipelined_ring_bcast") == 0) {
		int numofchunk = (int)(NUM_BYTES / chunk_size);
		if (rank == 0) {
			for (int i = 0; i < numofchunk; i++) {
//				printf("Chunk size = %d\n", chunk_size);
//				printf("Rank %d (running on '%s'): sending the message chunk %d\n", rank, hostname, i);
				MPI_Send(( & buffer[i * chunk_size]), chunk_size, MPI_CHAR, 1, 1, MPI_COMM_WORLD);
//				printf("Rank %d (running on '%s'): receiving the message chunk %d\n", rank, hostname, i);
//				MPI_Recv((&buffer[i*chunk_size]), chunk_size, MPI_CHAR, num_procs-1, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
//				printf("Rank %d (running on '%s'): receiving chunk %d DONE\n", rank, hostname, i);
			}
		} else {
			for (int i = 0; i < numofchunk; i++) {
				//				printf("Rank %d (running on '%s'): sending the message chunk %d\n", rank, hostname, i);
				MPI_Recv(( & buffer[i * chunk_size]), chunk_size, MPI_CHAR, rank - 1, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
				//				printf("Rank %d (running on '%s'): receiving the message chunk %d\n", rank, hostname, i);
				if (rank + 1 != num_procs) {
					MPI_Send(( & buffer[i * chunk_size]), chunk_size, MPI_CHAR, (rank + 1) % num_procs, 1, MPI_COMM_WORLD);
				}
				//				printf("Rank %d (running on '%s'): receiving chunk %d DONE\n", rank, hostname, i);
			}
		}
		...
		int numofchunk = (int)(NUM_BYTES / chunk_size);
		MPI_Request * reps = (MPI_Request * ) malloc(sizeof(MPI_Request) * numofchunk);
		MPI_Request * repr = (MPI_Request * ) malloc(sizeof(MPI_Request) * numofchunk);
		MPI_Status * stats = (MPI_Status * ) malloc(sizeof(MPI_Status) * numofchunk);
		if (rank == 0) {
			for (int i = 0; i < numofchunk; i++) {

				MPI_Isend(( & buffer[i * chunk_size]), chunk_size, MPI_CHAR, 1, 1, MPI_COMM_WORLD, & reps[i]);
//				printf("Rank %d (running on '%s'): sending the message chunk %d\n", rank, hostname, i);
//				MPI_Wait(&reps[i], &stats[i]);
			}
		} else {
			for (int i = 0; i < numofchunk; i++) {
				MPI_Recv(( & buffer[i * chunk_size]), chunk_size, MPI_CHAR, rank - 1, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
				if (rank + 1 != num_procs) {
					MPI_Isend(( & buffer[i * chunk_size]), chunk_size, MPI_CHAR, (rank + 1) % num_procs, 1, MPI_COMM_WORLD, & reps[i]);
//					printf("Rank %d (running on '%s'): sending the message chunk %d\n", rank, hostname, i);
//					MPI_Wait(&reps[i], &stats[i]);
				}
			}
		...
		;
	} else if (strcmp(bcast_implementation_name, "muitiport2lvfc") == 0){
